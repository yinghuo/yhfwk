...
//				Date dnDate=DateTimeUtil.addDate(fzzt.getTssj(),0,0,-21);
				//修改产前15天进入围产前期
				Date dnDate=DateTimeUtil.addDate(fzzt.getTssj(),0,0,-15);
				Date nowDate = new Date();
				if(nowDate.getTime()>=dnDate.getTime())
				{
					//进入围产期
					if(mrzt==null){
						mrzt = new NZMRZTXX();
						mrzt.setDay(0);
						mrzt.setNzxh(nzxx.getXh());
					}
					else if(mrzt.getZt()==NZMRZT.围产前期.getValue()){
						//判断是否已经是围产期
						break fz;
					}
					
					mrzt.setZt(NZMRZT.围产前期.getValue());
					session.saveOrUpdate(mrzt);
					session.flush();
					
					//@modify Daniel 2016-03-22 注释代码，妊娠期不进入围产前期，妊娠后期直接待产
...
//			System.out.println("牛只["+nzxx.getNzbh()+"]泌乳状态天数："+allCount);
			...
			//@modify 2016-03-22 Daniel 添加泌乳新状态，围产期
			if(mrzt.getZt()==NZMRZT.围产前期.getValue())
			{
//				System.out.println("  牛只处于围产前期：");
				//产前5天左右提示
			}
			else if(mrzt.getZt()==NZMRZT.围产后期.getValue())
			{
//				System.out.println("  牛只处于围产后期：");
				//围产后期15天
				int countDay=mrzt.getDay();//产犊后day更新
				if(countDay>15)
				{
					//进入泌乳期
					mrzt.setZt(NZMRZT.泌乳盛期.getValue());
//					session.saveOrUpdate(fzzt);
//					session.flush();
				}
			}			
			else if(mrzt.getZt()==NZMRZT.泌乳盛期.getValue())
			{
