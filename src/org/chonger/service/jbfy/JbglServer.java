...
/**   
*
* @title JbglServer.java 疾病信息管理业务逻辑
* @package org.chonger.service.jbfy
* @author Daniel
* @create 2015-5-15 
 */
@Service
@Transactional
public class JbglServer {
	@Autowired
	private CommonDAO<JBXX> dao;
	 ...
	 * @version V1.0
	 */
	public JBXX queryNZById(String id)
	{
		List<JBXX> resultList=dao.find(getQueryString(null, null)+" and model.xh='"+id+"'");
	...
	{
		String sql="from JBXX model where 1=1 ";
	...
	public void delete(String id) throws Exception{
		if(!StringUtil.IsEmpty(id))
		{
			...
			{
				String deleteHql="delete JBXX model where model.xh='"+id+"'";
	 ...
	 * @version V1.0
	 */
	public void saveOrUpdate(JBXX Jbxx)
	{
		if(Jbxx!=null) {
			//权限校验 
			User user=SessionUtils.getUser();
			if(user!=null&&user.getRole().getRtype()==2)
			{
				Jbxx.setNcbh(user.getNcjbxx().getXh());
			}
			
			if(StringUtil.IsEmpty(Jbxx.getXh()))
			{
				//牛只序号为空，表示新增，进行自动编号
				Jbxx.setXh(CommUUID.getUUID());
				dao.save(Jbxx);
			}
			else
				dao.saveOrUpdate(Jbxx);
		}
	}
}
