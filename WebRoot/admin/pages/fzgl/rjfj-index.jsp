...
<body>
				...
				<div class="fileButton" style="display:inline;">
	          		<form id="importform" style="display:inline;" action="${pageContext.request.contextPath}/master/data/import.action" method='post' enctype='multipart/form-data'>
			        	<input type="button" class="btn btn82 btn_export" value="导入">
			         	<input type="hidden" name="fileType" value="8">
			        	<input id="imgfile" class="fileInput cr-p" accept=".xlsx,.xls" type="file" name="upfile">
		        	</form>
	        	</div>
	        	<input type="button" name="button" data="8" onClick="dataExport()" class="btn btn82 btn_export" value="导出">
			</div>
		</div>
	</div>
					...
					<i:Page page="pager" count="9">
						...
						<s:else>
							<li class="first-child"><a
								href="${pageContext.request.contextPath}/master/fzgl/rjfj.action?p=${pageNowNum-1}${searchString}">上一页</a></li>
						</s:else>
						<i:PageNum>
							...
							<s:else>
								<li><a
									href="${pageContext.request.contextPath}/master/fzgl/rjfj.action?p=${pageIndex}${searchString}">${pageIndex}</a></li>
							</s:else>
						</i:PageNum>
						...
						<s:else>
							<li><a class=""
								href="${pageContext.request.contextPath}/master/fzgl/rjfj.action?p=${pageNowNum+1}${searchString}">下一页</a></li>
						</s:else>
	...
	<script>
     	...
     	*/%>
     	...
     	function edit(id)
     	{
     		window.location.href="${pageContext.request.contextPath}/master/fzgl/rjfj!edit.action?id="+id;
     	}
