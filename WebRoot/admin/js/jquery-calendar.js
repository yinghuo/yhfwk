...
var popUpCal = {
    ...
    setPos: function(targetObj, moveObj) {
        ...
        var eventLeft = $(targetObj).offset().left,
            eventTop = $(targetObj).offset().top,
            timeBoxHeight = 322,
            containerHeight = $(".centerRightContainer").height(),
            showTimeContentHeight = Number(containerHeight)-Number(eventTop);
        
        /*moveObj.css('position', 'absolute').css('left', coords[0] + 'px').
			css('top', (coords[1] + targetObj.offsetHeight) + 'px');*/
        
        moveObj.css('position', 'absolute').css('left', eventLeft + 'px');
        
        if(showTimeContentHeight < timeBoxHeight) {
        	var newTop = Number(eventTop)-timeBoxHeight+22;
        	moveObj.css('top', newTop + 'px');
        }
        else {
        	moveObj.css('top', eventTop + 'px');
        }
